...
do
  ...
  curl --header "$CONTENT_TYPE" --header "$ACCEPT" --data "@$TMP_ADOPTER" http://localhost:5000/api/1.0/adopter || exit 1
  rm -f $TMP_ADOPTER
  ...
  do
    ...
    curl --header "$CONTENT_TYPE" --header "$ACCEPT" --data "@$TMP_STAT" http://localhost:5000/api/1.0/statistic || exit 1
    echo "Creating Tobira content for statistic $stat_uuid for $adopter_uuid"
    curl --header "$CONTENT_TYPE" -d '{"statistic_key": "'$stat_uuid'", "data": { "num_realms": 3631, "num_blocks": 5558, "version": { "identifier": "v2.0", "build_time_utc": "Thu, 15 Jun 2023 11:26:19 +0000", "git_commit_hash": "18521b24e34cd11990f6b33913fb0d7a854ab0ec", "git_was_dirty": true }, "config": { "download_button_shown": true, "auth_mode": "login-proxy", "login_link_overridden": false, "logout_link_overridden": false, "uses_pre_auth": true, "has_narrow_logo": true } }}' http://localhost:5000/api/1.0/tobira || exit 1
    rm -f $TMP_STAT
    curl --header "Content-Type: application/json; utf-8" -X DELETE --data '{"statistic_key": "'$stat_uuid'"}' http://localhost:5000/api/1.0/tobira || exit 1
    curl --header "Content-Type: application/json; utf-8" -X DELETE --data '{"statistic_key": "'$stat_uuid'"}' http://localhost:5000/api/1.0/statistic || exit 1
  done

  curl --header "Content-Type: application/json; utf8" -X DELETE --data '{"adopter_key": "'$adopter_uuid'"}' http://localhost:5000/api/1.0/adopter || exit 1
done
